...
export const Card: React.FC<CardProps> = ({ children, className = '', ...props }) => (
  <div className={`bg-white dark:bg-stone-800 rounded-xl shadow-md border border-stone-300 dark:border-stone-700 p-6 ${className} card transition-colors duration-300`} {...props}>
    {children}
  </div>
);
...
export const Button: React.FC<ButtonProps> = ({ variant = 'primary', size = 'md', className = '', ...props }) => {
  const baseStyle = "font-medium rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed dark:focus:ring-offset-stone-900";
  
  const variants = {
    primary: "bg-[#D45D79] text-white hover:bg-[#C04865] focus:ring-[#D45D79] shadow-sm dark:bg-[#C04865] dark:hover:bg-[#A03550]", 
    secondary: "bg-stone-200 text-stone-800 hover:bg-stone-300 focus:ring-stone-500 dark:bg-stone-700 dark:text-stone-200 dark:hover:bg-stone-600",
    danger: "bg-red-50 text-red-700 hover:bg-red-100 focus:ring-red-500 border border-red-200 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800 dark:hover:bg-red-900/50",
    ghost: "text-stone-500 hover:text-stone-800 hover:bg-stone-100 dark:text-stone-400 dark:hover:text-stone-200 dark:hover:bg-stone-800"
  };
...
export const Input: React.FC<InputProps> = ({ label, error, helperText, suffix, className = '', ...props }) => {
  ...
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      ...
      {helperText && <p className="text-xs text-stone-500 dark:text-stone-500 italic">{helperText}</p>}
    </div>
  );
};
...
export const Select: React.FC<SelectProps> = ({ label, options, className = '', ...props }) => (
  <div className={`flex flex-col gap-1.5 ${className}`}>
    <label className="text-sm font-bold text-stone-700 dark:text-stone-300">{label}</label>
    <select
...
export const InfoTooltip = ({ text }: { text: string }) => (
  <div className="group relative inline-flex items-center ml-1 align-middle">
    <span className="cursor-help text-stone-400 hover:text-[#D45D79] dark:text-stone-500 dark:hover:text-rose-400">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
        <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
      </svg>
    </span>
    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-stone-800 dark:bg-white text-white dark:text-stone-900 text-xs rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all w-52 text-center z-50">
      {text}
      <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-stone-800 dark:border-t-white"></div>
    </div>
  </div>
);
